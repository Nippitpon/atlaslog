...
## 2026-09-15 — รอบ 52 (✅ SHIPPED, deploy main): กราฟ Body Composition ดูรวม 3 ค่าได้แล้ว — pill `ALL` เป็น %Δ

commit `770c9d7` (feat)

รอบ 51 ให้กราฟที่ดูได้**ทีละค่า** และเขียน comment ไว้เองที่ `BodyPage.tsx:88-89` ว่า
*"kg และ % ใช้แกน Y ร่วมกันไม่ได้ และ 75 kg ข้าง ๆ 33 kg จะบีบให้เส้นแบนทั้งคู่"* — จริงสำหรับ**ค่าดิบ**
แต่ทำให้ตอบคำถามหลักของ body recomposition ไม่ได้: **"น้ำหนักเท่าเดิม แต่กล้ามขึ้นเร็วกว่าไขมันลงไหม"**
ต้องกด pill สลับไปมาแล้วจำรูปกราฟเอาเอง · รอบนี้แปลงเป็น **% เทียบค่าแรกของแต่ละค่า** หน่วยจึงเท่ากันหมด
แกนเดียวกันได้อย่างซื่อสัตย์ ส่วนตัวเลขจริง kg/% ยังอ่านได้จากแถบ legend ใต้กราฟ

### ทำอะไร

- **`lib/bodyMetrics.ts`** — `BodySelection` (`BodyMeasure | 'all'`) · `normalizePct` · `buildNormalizedSeries`
  · `totalPctChange` (ค่าแรก→ค่าล่าสุด ต่างจาก `measureDelta` ที่เป็นสองแถวท้าย)
- **`charts/oneRMScale.ts`** — เพิ่ม **type `ChartSeries` อย่างเดียว ไม่มีการเปลี่ยน runtime** (อยู่ที่นี่เพื่อให้
  `lib/` สร้างซีรีส์ได้โดยไม่ต้อง type-import จาก `.tsx`)
- **`charts/MetricChart.tsx`** — จาก 1 ซีรีส์ → **N ซีรีส์** (`series: ChartSeries[]`) · โหมดค่าเดียวส่ง array
  ความยาว 1 จึงเป็น **โค้ดพาธเดียว ไม่มีสองสาขาให้เพี้ยน** · ไม่สร้างคอมโพเนนต์ที่สองมาก๊อปกัน
- **`features/profile/BodyPage.tsx`** — pill `ALL` (ขอบ/ตัวอักษร `var(--accent)` ตามแบบ `OneRMChart:45`
  — **ไม่ให้สีไลม์กับเส้นข้อมูล**) · `useState<BodySelection>('all')` เป็นค่าเริ่มต้น · แถบ legend โหมด ALL
  โชว์ `totalPctChange` แทน `measureDelta` · **คงกฎสีกลาง**ของเดลต้าไว้ (รอบ 51)

### ผลกระทบ (จัดการแล้ว)

- 🔬 **`PCT_STEPS` ที่วางแผนไว้ตอนแรก แย่กว่าไม่ทำ — ถอดออกทั้งหมด** · ตั้งใจเพิ่ม `[1, 2.5, 5, 10, 20, 25]`
  เพราะกลัว `FINE_STEPS` ตันที่ 10 แต่พอรันเลขเทียบจริงมันแพ้**ทุกเคส** (fill ของแกน):
  คัต `55.3%` vs `41.5%` · สัปดาห์แรก `20%` vs `8%` · ขาขึ้น `41.3%` vs `31%` · สุดขั้ว `76%` vs `50.7%`
  · เหตุ: `steps[0]=1` ดัน `floor = Math.min(1, steps[0])` เป็น 1 → `pad` ขั้นต่ำ 1 จุด =
  **บั๊กเส้นแบนที่รอบ 51 เพิ่งแก้ให้ kg** · และที่กลัวไว้ไม่จริง — `niceStep` fallback เป็นค่าท้ายสุดอยู่แล้ว
  step คา 10 แล้ว snap เป็นพหุคูณของ 10 (`-30..+10` รับ −22 ได้สบาย)
  → **ผลพลอยได้: `oneRMScale.ts` ไม่มีการเปลี่ยน runtime เลย regression pin ของ `/one-rm` แตะไม่ได้โดยโครงสร้าง**
- **เลิกใช้ prop `zeroLine`/`steps` → อนุมานเอาเอง** `signed = scale.vMin < 0 && scale.vMax > 0` ·
  ค่า kg/% ดิบเป็นบวกเสมอ แกนจึงไม่มีทางคร่อม 0 → โหมดค่าเดียวไม่ขึ้นเส้นประและไม่ติด `+` ให้ `75` ·
  ป้ายแกนใช้แฟล็กเดียวกัน · ถ้าวันหน้ามีคนส่งซีรีส์ normalize เดี่ยวเข้ามา ก็ได้ทรีตเมนต์ถูกโดยไม่ต้องบอก
- **เส้น baseline ไม่ต้องแก้ `makeScale`** — ทุกซีรีส์ที่ normalize แล้วมีจุด `0` เป๊ะ (จุดแรกของตัวเอง)
  และ `pad > 0` เสมอ → `vMin < 0 < vMax` การันตีโดยโครงสร้าง · ใช้ `((v - v0) / v0) * 100` ไม่ใช่
  `(v / v0 - 1) * 100` (ให้ 0 เป๊ะเหมือนกัน แต่เลี่ยง cancellation และอ่านออกว่าคือ percent change)
- **เส้นประ ไม่ใช่แค่สีเข้มกว่า** — light theme `--border-strong #d4d3cb` ห่างจาก `--border #e5e4dd`
  แค่เฉดเดียว ถ้าไม่ใส่ `strokeDasharray` มันจะอ่านเป็น **gridline เส้นที่สี่**
- 🐛 **เทสต์จับบั๊กจริง 1 ตัว** — guard `base <= 0` **ไม่กัน `NaN`** (เพราะ `NaN <= 0` เป็น `false`)
  ต้อง `!Number.isFinite(base)` · ไล่ปิดที่ `buildBodySeries` ด้วย (เดิมเช็ค `Number.isFinite(p.t)`
  แต่ **ไม่เคยเช็ค `p.value`**) — `NaN` ตัวเดียวทำ `Math.min/max` ใน `makeScale` พังทั้งกราฟ
- **gate ต้องดูซีรีส์ที่ยาวที่สุด ไม่ใช่ผลรวม** — 1 แถวที่กรอกครบ 3 ค่า = 3 จุด **ทับกันพิกเซลเดียว**
  ถ้า gate เป็น `pooled < 2` จะผ่านแล้ววาดจุดซ้อน 3 จุดโดยไม่มีเส้น → ใช้ `longest < 2`
- **`key={p.t}` บนวงกลมเป็นบั๊กที่มีอยู่แล้ว** — `isoFromYMD` anchor เที่ยงวัน สองแถววันเดียวกันได้ ISO
  ตรงกันเป๊ะ → key ชน → เปลี่ยนเป็น index (ยืนยันแล้วว่า console สะอาด)
- **caption ยุบรวมกับบรรทัด `ล่าสุด` เดิม ไม่เพิ่มแถวใหม่** — แถวใหม่ที่ขึ้นเฉพาะโหมด ALL จะทำ
  **การ์ดกระตุกทุกครั้งที่กด pill** · แถมปิดช่องโหว่เดิมด้วย: ตัวเลขเดลต้าที่ ship ไปรอบ 51 **ไม่มีป้ายบอกเลย**
  ว่าเทียบอะไร ตอนนี้เป็น `ล่าสุด 14/09/2026 · % เทียบค่าแรกของแต่ละค่า` / `· เทียบครั้งก่อน`

### verify

`pnpm test` **194 → 214 tests / 10 files** (`bodyMetrics.test.ts` +20 เคส: `normalizePct` ·
`buildNormalizedSeries` · `totalPctChange` · `makeScale on normalised %Δ data`) · `pnpm build` ผ่าน ·
ESLint สะอาด · **regression pin เดิม (`:128-135`) ยังเขียว**

**e2e จริงด้วย Playwright MCP ที่ 390px ทั้ง dark + light:**

| ทดสอบ | ผล |
|---|---|
| เปิด `/body` | ค้างที่ `ALL` · 3 เส้นแยกสีชัด บรรจบที่ 0 ทางซ้าย |
| pill 4 ตัว | **แถวเดียว 251/308px** ไม่ตก ไม่มี horizontal scroll |
| แกน Y โหมด ALL | `+10 / 0 / −10` ติดเครื่องหมาย · ทศนิยมไม่ถูกปัดทิ้ง |
| `%Δ` มุมขวาบน | **เป็น glyph จริง** (วัดได้ 4.2px เทียบ tofu 6.81px) ไม่ทับป้าย gridline |
| สลับกลับ `WEIGHT` | `77.5/75/72.5` **ไม่มี `+` ไม่มีเส้นประ** เหมือนรอบ 51 เป๊ะ · การ์ดไม่กระตุก |
| **1 แถวกรอกครบ 3 ค่า** | ขึ้น "มีข้อมูลจุดเดียว" **ไม่ใช่ 3 จุดทับกัน** |
| muscle ลงครั้งเดียว | จุดเขียวเดี่ยวเห็นชัดบน baseline ไม่ถูกเส้นประกลืน · fat ไม่เคยลง → legend `—` |
| 2 แถววันเดียวกัน | console **0 error** ไม่มี React duplicate-key warning |

✅ **บทเรียนรอบ 51 ใช้แล้วได้ผล** — seed ด้วย `useAppStore.setState()` **ไม่ใช่ `addBodyMetric`**
จึงไม่มี `syncBodyMetric` ยิงเลย **ตาราง `body_metrics` บนคลาวด์ไม่ถูกแตะแม้แถวเดียว** ·
hook ชั่วคราวใน `main.tsx` (expose store) ถอนคืนแล้ว · ภาพ screenshot ลบแล้ว
working tree เหลือแค่ 5 ไฟล์ที่ตั้งใจแก้

### ไม่ทำรอบนี้

- 🟡 **ช่อง `FAT%` โหมด ALL เป็นเปอร์เซ็นต์สัมพัทธ์** — 19.0 → 17.6 ขึ้น `−7.37%` แต่จริง ๆ ลง
  **1.4 จุด** · ตอนนี้กันด้วย caption + ตัวเลขใหญ่ที่ยังเป็น `17.6 %` · ถ้าใช้จริงแล้วยังสับสน
  ทางเลือกคือโชว์เป็นจุดเปอร์เซ็นต์เฉพาะช่องนั้น (แต่จะไม่ตรงกับเส้นในกราฟ) — **รอ feedback ก่อน**
- **ไม่แตะ `makeScale`** (มี regression pin คุม) · ไม่แตะ types / store / schema Supabase /
  persist key / `version`+`migrate` · ไม่แตะ `calories.ts` / `energy.ts`
- **ไม่ persist pill ที่เลือก** — ต้องเพิ่ม persist key ซึ่งรอบ 51 ก็เลี่ยงไว้ · ALL เป็น default แล้ว
- **`AthleteDetailPage`** (หน้าโค้ช) ยังมีแถบ body + `MiniBars` ก๊อปไว้ — รอบ 51 จดไว้แล้ว ยังไม่รวม
- **ไม่ยุบ `OneRMChart` เข้า `MetricChart` ที่ตอนนี้ general แล้ว** — `OneRMChart` ถือ pill row ของตัวเอง
  + เส้น manual/estimated แบบ dashed ด้วย ควรเป็นรอบแยก
- ไม่เพิ่ม tooltip / crosshair / เลือกช่วงเวลา — กราฟทั้งแอปไม่มี interaction เลย
- ไม่แตะงานค้าง 5 ข้อในตาราง 📌 (sync data-loss · accessory periodization · persist version/migrate
  · totalWeeks · coach edge function) — **ยังเปิดครบทั้ง 5**
...
**เทส scenario กับข้อมูลจริง (หลัง ship, ผู้ใช้สั่ง "test ให้หน่อย") — 22 เคสผ่านหมด**
harness ชั่วคราว (`vite build --ssr` + node + stub localStorage, ลบทิ้งแล้ว) รัน **`TWELVE_WEEK_PROGRAM`
ตัวจริง** (12×4 = 48 วัน) ผ่าน store จริง: ตั้ง `startDate` ให้วันนี้ตกใน week 3 แล้วมาร์กสัปดาห์ละ 3 วัน
ขาดวันที่ 4 ทุกสัปดาห์ = สภาพที่ผู้ใช้เจอเป๊ะ ๆ
- ก่อน skip → `9/48 วัน · 19%` · `Week 3/12` · แถวสัปดาห์ `3/4 DAYS` · W1–W3 เป็น `in_progress` ·
  โปรแกรม `active` **ยังไม่ปิด**
- ปุ่ม Skip โผล่ถูกจุด: W1/W2 ทุกวัน + W3 Mon/Tue/Thu เป็น PAST แต่ **W3 Fri = วันนี้ ไม่มีปุ่ม** ·
  สัปดาห์อนาคตไม่มีปุ่ม
- กด skip วันค้าง W1+W2 → สองสัปดาห์เป็น `done`, `doneWeeks` 0→2, **`pct` ยัง 19% ไม่ขยับ**,
  `skippedDays` = 2, `remainingDays(W1)` = 0
- Un-skip → W1 กลับ `in_progress` + วันค้างกลับมา
- **เคสที่ผู้ใช้ย้ำ**: เทรนครบยกเว้นวันสุดท้ายของ W12 → ไม่แตะ = `active` ยังไม่ปิด ·
  กด Skip = `completed` แต่แถบได้ `94%` (45 เทรน + 3 skip) ไม่ใช่ 100%
...
**ตรรกะยืนยันครบแล้วด้วย scenario ข้างบน — เหลือแค่หน้าตา** ที่ยังไม่ได้ตาเห็น (แอป gate ด้วย Supabase auth
และเครื่องนี้ไม่มี Playwright): ตำแหน่งปุ่ม Skip ใน footer การ์ดวัน (อยู่ข้าง Edit), badge `SKIPPED`,
บรรทัด `· N skipped` ใต้ Progress, และการ์ดวันที่ skip ที่จาง 0.7
