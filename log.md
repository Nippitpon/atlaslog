...
| 4 | Social: Coach-Athlete + Program sharing + In-app reminder | ✅ Backend setup + e2e 7/7 ผ่าน (2026-06-22) — เหลือ merge PR → prod |
...
> **สถานะ:** ✅ **Backend setup + e2e ครบ 7/7 ผ่านหมด (2026-06-22)** — เหลือแค่ merge PR → prod
> (Code + build + lint + security check + push เสร็จตั้งแต่ 2026-06-21). แผนเต็ม: `~/.claude/plans/log-md-encapsulated-leaf.md`
>
> **commit `bf3020f`** บน branch `phase4-social-coach-sharing` (push ขึ้น origin แล้ว 2026-06-21)
> — เปิด PR ได้ที่ https://github.com/Nippitpon/atlaslog/pull/new/phase4-social-coach-sharing
> (ยังไม่ merge เข้า main — STEP 4 ด้านล่าง)
...
**bug เล็กที่เจอ (low, ควรแก้):** ปุ่ม **Share program** (`aria-label="Share program"`) ถูกวางใน `<button>`
การ์ด custom program → invalid DOM nesting (`<button>` ซ้อน `<button>`) → React hydration warning ใน console
ProgramsPage. ใช้งานได้ปกติ (คลิกได้) แต่ควรเปลี่ยนการ์ดจาก `<button>` เป็น `<div role=button>` หรือย้ายปุ่ม Share ออกนอก

### 🔜 STEP 4 — Merge + deploy (RESUME HERE)
- [ ] เปิด PR `phase4-social-coach-sharing` → merge เข้า `main`
- [ ] Vercel auto-deploy → ทดสอบ prod ซ้ำ (linking + CORS ผ่านโดเมน prod เหมือน Phase 3.5)
- [ ] (optional) แก้ bug `<button>` ซ้อนใน ProgramsPage ก่อน merge
