...
| 4 | Social: Coach-Athlete + Program sharing + In-app reminder | 🟡 Code เสร็จ — รอ setup Supabase (SQL + deploy `coach` fn) + e2e |
...
> **สถานะ:** 🟡 Code + build + lint + security check เสร็จ + **push แล้ว** — **รอ setup Supabase backend**
> (รัน SQL + deploy edge function `coach`) ถึงจะ e2e ได้. แผนเต็ม: `~/.claude/plans/log-md-encapsulated-leaf.md`
>
...
> (ยังไม่ merge เข้า main — รอ backend setup + e2e ก่อน)
